...
// ✅ Normalize expenses properly
  ...
  pdf.text('FinLoAn-AI - Expense Report', pageWidth / 2, yPos, { align: 'center' });
        yPos += 15;
  ...
  const filename = `finloan-report-${getFilenameDatePart()}.pdf`;
  ...
  exportToCSV(csvData, `finloan-expenses-${period}-${new Date().toISOString().split('T')[0]}.csv`);
    } catch (error) {
